module.exports = {
  ...
  onChat: async function ({ api, event }) {
    try {
      ...
      // FB's reaction endpoint rejects some emoji when the trailing
      // variation-selector (U+FE0F) is present, e.g. "❤️" -> reject,
      // "❤" -> accept. Strip it before sending.
      const reactToSend = react.replace(/\uFE0F/g, "");

      // ✅ FINAL FIX — NO callback, NO true
      api.setMessageReaction(reactToSend, messageID);
