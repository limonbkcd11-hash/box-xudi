...
const Boolean_Option = ['online', 'selfListen', 'listenEvents', 'updatePresence', 'forceLogin', 'autoMarkDelivery', 'autoMarkRead', 'listenTyping', 'autoReconnect', 'emitReady', 'reactionDebug'];
...
function buildAPI(globalOptions, html, jar) {
    ...
    // NOTE: this used to resolve config.json via process.cwd() only, which is
    // the directory Node was *launched from* — not necessarily this package's
    // folder. Under pm2/docker/a bot-panel that starts the process from a
    // different working directory, config.json was silently never found, the
    // whole block was wrapped in an empty catch(){}, and E2EE just never
    // turned on — with zero indication why. Now we check both locations and
    // always log the outcome instead of failing silently.
    try {
        const _e2eeCandidates = [
            path.join(process.cwd(), 'config.json'),
            path.join(__dirname, 'config.json')
        ];
        const _e2eeRootPath = _e2eeCandidates.find(p => fs.existsSync(p));
        if (_e2eeRootPath) {
            ...
            log.info("index", "E2EE config loaded from " + _e2eeRootPath + " → enableE2EE=" + !!globalOptions.enableE2EE);
        } else {
            log.warn("index", "E2EE: no config.json found in " + _e2eeCandidates.join(" or ") +
                " — E2EE will stay OFF unless enableE2EE:true is passed directly to fca(). Current: " +
                !!globalOptions.enableE2EE);
        }
    } catch (_e2eeErr) {
        log.error("index", "E2EE: failed to load config.json — E2EE staying OFF:", _e2eeErr && _e2eeErr.message ? _e2eeErr.message : _e2eeErr);
    }
