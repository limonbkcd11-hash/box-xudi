...
// ─── Crash safety net ───────────────────────────────────────────────────────
// Logs unexpected errors instead of letting the process die silently.
process.on('unhandledRejection', (reason) => {
    console.error('[BOT] Unhandled rejection:', reason && reason.message || reason);
});
process.on('uncaughtException', (err) => {
    console.error('[BOT] Uncaught exception:', err && err.message || err);
});
...
async function runTests(api, threadID) {
    ...
    const steps = [
        {
            label: '1/5 Text',
            run: () => send(api, tid, 'Hello from HRIDOY-FCA v1.0.28! ✅')
        },
        {
...
async function main() {
    ...
    fca({ appState }, {
        ...
        autoMarkRead: false,
        autoReconnect: true,
        enableE2EE: true   // don't rely only on config.json auto-detect — force it on here too
    }, async (err, api) => {
        if (err) {
        ...
        console.log(`[BOT] E2EE enabled: ${api.getEnableE2EE ? api.getEnableE2EE() : 'true (requested)'}`);
