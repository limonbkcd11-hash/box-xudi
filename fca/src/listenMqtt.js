...
function printMqttBanner(region, autoReconnect) {
    ...
    var rows = [
        titleClr + '  ✅  HRIDOY-FCA MQTT Connected' + rst,
        '',
        labelClr + '  📍  Region         ' + rst + valClr + regionVal + rst,
        labelClr + '  🔄  Auto-reconnect  ' + rst + reconnVal,
        urlClr   + '  🌐  github.com/hridoy-dev/HRIDOY-BOT' + rst,
        '',
        accentClr + '  💎  Author  HRIDOY' + rst
    ];
...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
    ...
    function buildStream() {
        ...
        ws.on('error', function (err) {
            // Still must have a listener here or Node crashes the process on
            // an unhandled 'error' event — but log it instead of discarding
            // it silently, since a WebSocket-level error is exactly the kind
            // of thing that used to disappear without a trace.
            log.warn("listenMqtt", "underlying WebSocket error: " + (err && err.message ? err.message : err));
        });
    ...
    // ─── Health monitor ─────────────────────────────────────────────────────
    // Exposed on global.GoatBot.fcaHealth (when the bot core is present) and
    // ctx.health regardless, so diagnostics/tests can inspect transport state
    // without depending on any single event having recently fired. Reaction
    // inactivity is NEVER treated as a health signal here — only transport
    // level activity (packets, connect/close, reconnects) counts.
    var health = ctx.health || {
        ...
        lastReactionAt: null,
        reconnectCount: 0,
        reconnectHistory: [], // timestamps, trimmed to the last hour
        currentBackoffMs: 3000
    };
    ctx.health = health;
    if (typeof global.GoatBot === "object" && global.GoatBot) global.GoatBot.fcaHealth = health;
    health.mqttState = "connecting";

    // ─── Reconnect guard ────────────────────────────────────────────────────
    // mqtt.js's own reconnectPeriod is disabled (we use a custom duplex over
    // a raw WebSocket), so WE are responsible for noticing every way the
    // connection can die: an explicit 'error', a clean 'close' (FB idles out
    // the socket after a while with no error at all — this is why reactions/
    // messages silently stopped arriving after some time), and 'offline'.
    // Without a handler on 'close'/'offline' the client just sits there dead.
    //
    // Backoff: exponential with a cap and jitter, so many bots reconnecting
    // after a shared outage don't hammer Facebook's edge at the exact same
    // instant (reconnect storm). The math lives in ./reconnectPolicy.js so it
    // can be unit tested in isolation — see diagnostics/reconnect-policy.test.js.
    var reconnectPolicy = require("../reconnectPolicy.js").createReconnectPolicy();

    var _reconnectScheduled = false;
    var _stableTimer = null;

    function scheduleReconnect(reason) {
        if (_reconnectScheduled) return; // avoid duplicate/overlapping reconnects
        _reconnectScheduled = true;      // when error+close fire back-to-back
        health.mqttState = "reconnecting";
        if (_stableTimer) { clearTimeout(_stableTimer); _stableTimer = null; }
        stopMqttSpinner();

        health.reconnectCount++;
        var recentReconnects = reconnectPolicy.recordReconnect();
        health.reconnectHistory = reconnectPolicy.getReconnectHistory();
        var delay;
        if (reconnectPolicy.isCircuitBreakerTripped(recentReconnects)) {
            delay = reconnectPolicy.cooldownMs();
            log.warn("listenMqtt", recentReconnects + " reconnects in the last few minutes" +
                " — circuit breaker engaged, waiting " + (delay / 1000) + "s before trying again. This usually means a persistent network/account problem, not a transient blip.");
        } else {
            delay = reconnectPolicy.nextBackoff();
            log.warn("listenMqtt", "MQTT disconnected (" + reason + "). Reconnecting in " + Math.round(delay / 1000) + "s...");
        }
        health.currentBackoffMs = reconnectPolicy.getCurrentBackoffMs();

        try { mqttClient.end(true); } catch (e) { /* already closed */ }
        setTimeout(function () {
            _reconnectScheduled = false;
            if (ctx.globalOptions.autoReconnect) getSeqID();
            else globalCallback({ type: "stop_listen", error: "Connection closed: " + reason }, null);
        }, delay);
    }
    ...
    mqttClient.on('close', function () { health.mqttState = "closed"; scheduleReconnect('close'); });
    mqttClient.on('offline', function () { health.mqttState = "offline"; scheduleReconnect('offline'); });
    mqttClient.on('reconnect', function () { });

    // ─── Half-open watchdog ─────────────────────────────────────────────────
    // Some networks/proxies let the TCP socket go "half-open": FB stops
    // sending data but never sends a FIN, so neither 'error' nor 'close'
    // ever fires and the client just sits there looking connected while
    // actually being dead (the classic "works fine, then quietly stops
    // after a while" symptom). Force a reconnect if we've heard nothing —
    // not even a keepalive pingresp — for 2x the keepalive interval.
    //
    // Deliberately based on ANY packet activity, never on whether a reaction
    // specifically has happened — reactions can legitimately be silent for
    // hours without anything being wrong.
    health.lastPacketAt = Date.now();
    mqttClient.on('packetreceive', function () { health.lastPacketAt = Date.now(); });
    var _watchdog = setInterval(function () {
        if (mqttClient.disconnected || mqttClient.disconnecting) { clearInterval(_watchdog); return; }
        if (ctx.globalOptions.reactionDebug) {
            log.info("reactionDebug", "watchdog tick | mqttState=" + health.mqttState +
                " | msSinceLastPacket=" + (Date.now() - health.lastPacketAt) +
                " | msSinceLastReaction=" + (health.lastReactionAt ? (Date.now() - health.lastReactionAt) : "never") +
                " | reconnectCount=" + health.reconnectCount +
                " | mqttClient.listenerCount(message)=" + mqttClient.listenerCount("message") +
                " | mqttClient.listenerCount(error)=" + mqttClient.listenerCount("error"));
        }
        if (Date.now() - health.lastPacketAt > (mqttOptions.keepalive * 1000 * 2)) {
            clearInterval(_watchdog);
            scheduleReconnect('watchdog-timeout');
        }
    }, 15000);
    mqttClient.once('close', function () { clearInterval(_watchdog); if (_stableTimer) clearTimeout(_stableTimer); });

    mqttClient.on('connect', function () {
        health.mqttState = "connected";
        // Re-subscribe to every required topic on every (re)connect, including
        // after an automatic recovery — Facebook does not remember previous
        // subscriptions across a fresh MQTT session.
        topics.forEach(function (topic) { mqttClient.subscribe(topic); });

        // A connection that survives 60s is treated as stable: drop the
        // backoff back to the base delay so a later, unrelated disconnect
        ...
        ctx.tmsWait = function () {
            ...
            if (ctx.globalOptions.enableE2EE === true) {
                log.info("listenMqtt", "E2EE: bridge connecting...");
                var bridge = e2eeBridge.createBridge(ctx);
                if (global.GoatBot) global.GoatBot._e2eeBridge = bridge;
                bridge.connect(globalCallback).then(function () {
                    log.info("listenMqtt", "E2EE: bridge connected.");
                }).catch(function (err) {
                    log.error("listenMqtt", "E2EE bridge connect error:", err && err.message ? err.message : err);
                });
            } else {
                log.warn("listenMqtt", "E2EE: disabled (globalOptions.enableE2EE is not true) — E2EE/secret threads will not be detected.");
        ...
        if (clientPayload && clientPayload.deltas) {
            for (var i in clientPayload.deltas) {
                ...
                if (delta.deltaMessageReaction) {
                    if (ctx.globalOptions.reactionDebug) {
                        log.info("reactionDebug", "raw deltaMessageReaction: " + JSON.stringify(delta.deltaMessageReaction));
                    }
                    if (!ctx.globalOptions.listenEvents) {
                        if (ctx.globalOptions.reactionDebug) {
                            log.warn("reactionDebug", "message_reaction dropped: globalOptions.listenEvents is false " +
                                "(toggle it back on via the bot's FCA options / settings command if reactions should fire).");
                        }
                    } else {
                        (function () {
                            var reactionThreadKey = delta.deltaMessageReaction.threadKey || {};
                            var reactionThreadID = reactionThreadKey.threadFbId || reactionThreadKey.otherUserFbId;
                            var reactionEvent = {
                                type: "message_reaction",
                                threadID: reactionThreadID != null ? reactionThreadID.toString() : undefined,
                                messageID: delta.deltaMessageReaction.messageId,
                                reaction: delta.deltaMessageReaction.reaction,
                                senderID: delta.deltaMessageReaction.senderId != null ? delta.deltaMessageReaction.senderId.toString() : undefined,
                                userID: delta.deltaMessageReaction.userId != null ? delta.deltaMessageReaction.userId.toString() : undefined
                            };
                            if (!ctx.health) ctx.health = {};
                            ctx.health.lastReactionAt = Date.now();
                            if (ctx.globalOptions.reactionDebug) {
                                log.info("reactionDebug", "dispatching message_reaction: " + JSON.stringify(reactionEvent));
                            }
                    ...
                    log.error("listenMqtt", "Failed to process a ClientPayload delta (reaction/unsend/reply):", deltaErr);
                }
            }
            return;
        }
...
// We never call setMaxListeners(0) / raise limits to hide leak warnings —
// per the audit requirements, Node's default MaxListenersExceededWarning is a
// genuine diagnostic and should be surfaced, not muted. This just routes it
// through the same logger instead of letting it go to raw stderr unnoticed.
// Guarded so it only attaches once even though this module can be
// require()'d fresh per listenMqtt() call in some setups.
if (!global.__fcaListenerWarningHookAttached) {
    global.__fcaListenerWarningHookAttached = true;
    process.on("warning", function (warning) {
        if (warning && warning.name === "MaxListenersExceededWarning") {
            log.warn("listenerLeak", warning.message + " — investigate before increasing any listener limit.");
        }
    });
}
