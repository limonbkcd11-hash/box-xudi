...
// Auto-expiring Map: entries set via .set() get stamped with a creation
// time; a background sweep removes anything older than ttlMs. Used for
// onReaction/onReply so a "please react to this" prompt that nobody ever
// reacts to doesn't sit in memory forever on a long-running bot.
class ExpiringMap extends Map {
	constructor(ttlMs = 15 * 60 * 1000, sweepMs = 5 * 60 * 1000) {
		super();
		this.ttlMs = ttlMs;
		this._createdAt = new Map();
		const timer = setInterval(() => this._sweep(), sweepMs);
		if (timer.unref) timer.unref();
	}
	set(key, value) {
		this._createdAt.set(key, Date.now());
		return super.set(key, value);
	}
	delete(key) {
		this._createdAt.delete(key);
		return super.delete(key);
	}
	_sweep() {
		const now = Date.now();
		for (const [key, time] of this._createdAt) {
...
global.GoatBot = {
	...
	onReply: new ExpiringMap(), // store all onReply (auto-expires after 15 min)
	onReaction: new ExpiringMap(), // store all onReaction (auto-expires after 15 min)
